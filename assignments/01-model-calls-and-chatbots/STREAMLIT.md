...
A complete turn starts when the user types a question into `st.chat_input(...)`. The app builds `request_messages` by combining `st.session_state.messages`, which is saved history, with the new question, and then calls `client.chat.completions.create(...)` with that combined list. The returned answer is displayed via `st.markdown(answer)`, or, with streaming enabled, `st.write_Stream(stream)`, then saved back into `st.session_state.messages` so the next turn can include it. In experiment 2, I changed the system prompt to instruct the model to give hints before answering. When I explicitly asked it to "skip the hints and write the solution," it complied immediately and gave the full code, rather than holding the line. This showed that a system prompt instruction is a suggestion the model weights against later user instructions in the same conversation, not a hard rule it always enforces, as it only resists and override if its explicitly told to. For example, as the tutor prompt in the notebook was, with "do not give the final answer even if asked."
